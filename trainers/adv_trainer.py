...
class BlackBoxAdvTrainer_fgsm:
    ...
    def train_epoch(self, train_data, epoch_num):
        ...
        if self.args.attack_type == "random":
            ...
            # Normalize the adversarial gradient: with l2-norm, this becomes
            # steepest descent in l2 which makes convergence faster.
            normalized_adv_grads = adv_grads / adv_grads.norm(p=2, dim=1, keepdim=True)  # torch.Size([131, 14007])

            # 扰动大小eps
            eps = 0.1
            normalized_adv_grads = normalized_adv_grads * eps

            normVal = normalized_adv_grads.norm(p=2, dim=1, keepdim=True)
            mask = normVal<=eps
            scaling = eps/normVal
            scaling[mask] = 1
            normalized_adv_grads = normalized_adv_grads*scaling
...
class BlackBoxAdvTrainer_pgd:
    ...
    def train_epoch(self, train_data, epoch_num):
        ...
        def _back_grad(sur_trainer, new_fake):
        # Reset the surrogate model.
            sur_args = Bunch(self.args.surrogate)
            sur_args.model_load_path = ""
            # sur_trainer_class = sur_args.model["trainer_class"]
            ...
            evl_data_tensor = torch.cat(
                [sparse2tensor(train_data).to(self.device),
                    new_fake.detach().clone().to(self.device)], dim=0)
            evl_data_tensor.requires_grad_()
        
            adv_grads = sur_trainer.evl(
                data_tensor=evl_data_tensor,
                n_fakes=self.n_fakes,
                target_items=self.target_items
            )
            return adv_grads
        ...
        if self.args.attack_type == "random":
            ...
            this_fake_tensor = self.fake_tensor
            for i in range(pgd_step):

                print("start pgd attack {}".format(i+1))

                adv_grads = _back_grad(sur_trainer, new_fake = new_fake_tensor)
                if self.args.click_targets:
                    adv_grads[:, self.target_items] = 0.0
                normalized_adv_grads = adv_grads / adv_grads.norm(p=2, dim=1, keepdim=True)
                ...
                eps = 0.1
                stepsize = 0.005
                normalized_adv_grads = normalized_adv_grads * stepsize
                ...
                print("chang tensor : {}".format((new_fake_tensor - this_fake_tensor).sum()))
                this_fake_tensor = new_fake_tensor
